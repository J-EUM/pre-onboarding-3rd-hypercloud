...
- signup 기능
  ...
  - name, email, password 입력 받아서 bcrypt 사용하여 비밀번호 암호화 후 users 테이블에 저장

- login 기능

  - email, password 입력 받아서 올바른 email인지 확인 후 아닌 경우 에러 발생
  - 입력 받은 비밀번호와 저장된 비밀번호가 같은 경우 jwt를 사용하여 토큰 발행, 토큰 유호시간은 하루

- changePassword 기능
  - token, email, password 입력 받아서 로그인 한 유저의 email이 입력 받은 email과 같은지 확인 후 다르면 에러 발생
  - 같을 경우 signup과 같은 방식으로 입력 받은 비밀번호를 암호화 후 비밀번호 update
...
회원가입, 로그인, 비밀번호 변경 -> https://documenter.getpostman.com/view/22723177/2s83zpKgQa
